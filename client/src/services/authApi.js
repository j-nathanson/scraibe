...
const url = 'http://18.118.122.152:8080/security'

// Find way to be able to verify w/ email
// Maybe we have a function that first does getByEmail to get user name then tries to authenticate?
export async function authenticate(credentials) {
  ...
  const response = await fetch(`${url}/authenticate-username`, init);
...
const makeUserFromJwt = (jwtToken) => {
  ...
  if (tokenParts.length > 1) {
    ...
    return {
      userId: decodedUserData.app_user_id,
